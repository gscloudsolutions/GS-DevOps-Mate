 ...
 */
export function getTestSubmission(testType, alias, filePath, testClasses = null, wait) {
    return new Promise((resolve) => {
        ...
        logger.debug(
            "runApexTests.js ::",
            "Running command ::",
            `SFDX_IMPROVED_CODE_COVERAGE='true' sfdx force:apex:test:run -u ${alias} -l ${testType} -r json -d ${filePath} -c -w ${wait} ${
                testType === TestLevel.SPECIFIED_TESTS ? `-n ${testClasses}` : ""
            }`
        );

        const submission = shellJS.exec(
            `SFDX_IMPROVED_CODE_COVERAGE='true' sfdx force:apex:test:run -u ${alias} -l ${testType} -r json -d ${filePath} -c -w ${wait} ${
                testType === TestLevel.SPECIFIED_TESTS ? `-n ${testClasses}` : ""
            }`,
            {
 ...
 * @return void
 */
export async function renameFiles(targetDir, testRunId, buildNumber) {
    return new Promise((resolve, reject) => {
        try {
            fs.renameSync(`${targetDir}/test-result-${testRunId}.json`, `${targetDir}/test-result-${buildNumber}.json`);
            fs.renameSync(
                `${targetDir}/test-result-${testRunId}-junit.xml`,
                `${targetDir}/test-result-${buildNumber}-junit.xml`
            );
 ...
 */
export async function checkTestCoverage(result, minimum = 75) {
    return new Promise((resolve, reject) => {
        try {
            ...
            if (testCoverage < minimum) {
                logger.debug(
                    "runApexTests.js :: ",
                    "checkTestCoverage :: ",
                    `test coverage for specified tests is <${minimum}`
                );
