...
import { createDestructiveManifest, createPackageManifest, getMetadataTypesWithMetaFile } from "../utils/manifestUtil";
...
function getSFDXModulesList(repoPath) {
...
function generateGitDiffList(output) {
    let totalDiffs = 0;
    ...
    logger.debug(
        ...
        modifiedItems.length + renamedItems.length + newItems.length + deletedItems.length
    );
}
 ...
 * @param {*} sfdxrepo
 */
async function prepareDiffItemsList(repoPath, nextCommit, previousCommit, sfdxrepo) {
        ...
        try {
            ...
            if (sfdxrepo === "true" || sfdxrepo === true) {
                ...
                modulesInfo.forEach((element) => {
                    ...
                    const output = shellJS.exec(diffCommand, { silent: false }).stdout;
                ...
                logger.debug(diffCommand);
                const output = shellJS.exec(diffCommand, { silent: true }).stdout;
            ...
            if (allDIffComps.length === 0) {
                logger.debug(
                    "No diffs found, try with some other commit-SHAs or tags or try a full no-diff package creation"
                );
                process.exit(21);
            }
...
async function createProjectJSON(repoPath, diffProjectPath) {
    return new Promise((resolve, reject) => {
        try {
            // Reading the sfdx-project.json as the dependencies file
            const packageObj = fsExtra.readJSONSync(`${repoPath}/sfdx-project.json`);
...
async function copyDiffContent(repoPath, diffProjectPath) {
    return new Promise((resolve, reject) => {
        try {
            logger.debug("diffProjectPath: ", diffProjectPath);
            logger.debug("modifiedItems: ", modifiedItems, " length: ", modifiedItems.length);
            logger.debug("renamedItems: ", renamedItems, " length: ", renamedItems.length);
            logger.debug("newItems: ", newItems, " length: ", newItems.length);
            logger.debug("deletedItems: ", deletedItems, " length: ", deletedItems.length);
            ...
            allDIffComps.forEach((element) => {
                ...
                if (
                    extensionNamesForCmpsWithMetaFiles.includes(path.extname(element)) &&
                    path.basename(path.dirname(element)) !== "sites"
                ) {
                    logger.debug(
                        `${element} needs special treatment as it needs it's -meta.xml file to be considered as well`
                    );
                ...
                if (
                    ...
                    path.extname(element) === ".jpg" ||
                    path.extname(element) === ".html"
                ) {
                    ...
                    if (
                        path.dirname(path.dirname(element)).includes("aura") ||
 ...
 */
export async function prepareDiffProject(
    repoPath,
        ...
        try {
            ...
            prepareDiffItemsList(repoPath, nextCommit, previousCommit, sfdxrepo)
                .then((message) => {
                    ...
                    return getMetadataTypesWithMetaFile();
                })
                .then((response) => {
                ...
                .then((message) => {
                    ...
                    return createPackageManifest(artifactslocation);
                })
                .then((message) => {
                    ...
                    if (CREATE_DESTRUCTIVE_XML === true || CREATE_DESTRUCTIVE_XML === "true") {
                        ...
                        return createDestructiveManifest(deletedItems, artifactslocation);
                    } else {
