...
function transformAndSortBySeverity(results) {
...
export async function scanCode(targetPathforScanning, pmdConfigPath, eslintConfigPath, SLACK_WEBHOOK_URI, NOTIF_TITLE) {
    ...
    if (SLACK_WEBHOOK_URI) {
        if (isSCAFailed) {
            await sendFailureMessage(SLACK_WEBHOOK_URI, severitySummary, NOTIF_TITLE);
        } else {
            await sendSuccessMessage(SLACK_WEBHOOK_URI, severitySummary, NOTIF_TITLE);
        }
    }
    return isSCAFailed;
}
