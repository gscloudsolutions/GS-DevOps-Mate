...
export function createPackageVersionCommand(command) {
    try {
...
export async function getVersionReport(result) {
...
export async function createPackageVersion(command) {
    return new Promise((resolve, reject) => {
        try {
            ...
            if (command.targetdevhubusername && command.targetdevhubpassword) {
                loginWithCredentials(command.targetdevhubusername, command.targetdevhubpassword, "PRODUCTION").then(
                    (connection) => {
...
export async function generateSuccessfulPackageCreationMessage(successMessage) {
    const blocks = [];
...
export async function generatePackageCreationFailureMessage(error) {
...
export async function handlePackageCodeCoverage(report, minRequiredCodeCoverage) {
    return new Promise((resolve, reject) => {
        logger.debug("report: ", report);
