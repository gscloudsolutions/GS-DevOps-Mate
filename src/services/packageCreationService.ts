...
import { createPackageManifest } from "../utils/manifestUtil";
import { prepareDiffProject } from "./gitDiff";
import { sendFailureMessage } from "./scaSlackNotifService";

init();

const MULTI_PACKAGE_NAME = process.env.MDAPI_PACKAGE_GROUP_NAME
    ? process.env.MDAPI_PACKAGE_GROUP_NAME
    : "metadatapackage-group";
...
async function promiseMethod(message) {
    return new Promise((resolve) => {
        resolve(message);
    });
}

export function createPackage(command, type, artifactCreationMethod) {
    ...
    if (!oldCommit && newCommit) {
        ...
        logger.debug(
            `${type}: deploymentInfo.json path with name of the file : ${deploymentInfoPath}/deploymentInfo_${command.buildId}.json`
        );
        if (fs.existsSync(`${deploymentInfoPath}/deploymentInfo_${command.buildId}.json`)) {
            const deploymentInfoObject = fs.readJSONSync(
                `${deploymentInfoPath}/deploymentInfo_${command.buildId}.json`
            );
    ...
    const packageVersion = createPackageVersions(command.packageVersion, oldCommit, newCommit, projectLocation);
    logger.debug(`${type}:  packageVersion: `, packageVersion);

    // Create package only if it does not exists
    const packageNameWithoutVersion =
        type === "source-combined" || type === "mdapi" ? MDAPI_PACKAGE_NAME : MULTI_PACKAGE_NAME;
    if (fs.existsSync(`${artifactsLocation}/${packageNameWithoutVersion}-${packageVersion}`)) {
        ...
        promiseMethod(`${type}`)
            ...
            .catch(async (error) => {
                ...
                if (SLACK_WEBHOOK_URL) {
                    await sendFailureMessage(SLACK_WEBHOOK_URL, error.message);
                    process.exit(1);
                }
                process.exit(1);
            });
        ...
        prepareDiffProject(projectPath, diffProjectPath, command.newCommit, oldCommit, sourceFormat, artifactsLocation)
            .then((message) => {
                ...
                return artifactCreationMethod(projectLocation, artifactsLocation, packageVersion, doGitDiff);
            })
            .then((message) => {
                logger.debug(`${type}: `, message);
                process.exit(0);
 ...
 */
export async function createMultipleArtifacts(srcProjectPath, locationToStoreArtifacts, packageVersion) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            // can work
            fs.writeJson(
                `${srcProjectPath}/tempSFDXProject/sfdx-project.json`,
                {
                    packageDirectories: [
                        {
                            path: "tempModule",
          ...
          to ${srcPath}/tempSFDXProject/tempModule/main/default`);
                        if (fs.existsSync(`${srcPath}/${element.path}/main/default`)) {
                            logger.debug(
                                `${srcPath}/${element.path} exists and copying it to ${srcPath}/tempSFDXProject/tempModule/main/default`
                            );
                            fs.copySync(
                                `${srcPath}/${element.path}/main/default`,
                                `${srcPath}/tempSFDXProject/tempModule/main/default`
                            );

                            if (fs.existsSync(`${srcPath}/.forceignore`)) {
                                logger.debug(
                                    `${srcPath}/.forceignore exists and  and copying it to ${srcPath}/tempSFDXProject`
                                );
                                fs.copySync(`${srcPath}/.forceignore`, `${srcPath}/tempSFDXProject/.forceignore`);
                            }
                            ...
                            const output = shellJS.exec("sfdx force:source:convert --json", { silent: false });
                            ...
                            if (locationToStoreArtifacts) {
                                logger.debug(
                                    `Copying the mdapi package (artifact) to the artifacts location - ${locationToStoreArtifacts} and renaming it to have package version as well`
                                );
                                fs.ensureDirSync(`${locationToStoreArtifacts}/${MULTI_PACKAGE_NAME}-${currentVersion}`);
                                fs.copySync(
                                    `${mdapiPackageLocation}`,
                                ...
                                logger.debug(
                                    `Renamed module: ${locationToStoreArtifacts}/${MULTI_PACKAGE_NAME}-${currentVersion}/${moduleName}-${currentVersion}`
                                );
                            }
                        }
                    });
                    ...
                    zipFolder.zipFolder(
                        `${locationToStoreArtifacts}/${MULTI_PACKAGE_NAME}-${currentVersion}`,
                        `${locationToStoreArtifacts}/${MULTI_PACKAGE_NAME}-${currentVersion}.zip`,
                        (error) => {
                            if (error) {
 ...
 * @param {*} packageVersion
 */
export async function createCombinedArtifact(srcProjectPath, locationToStoreArtifacts, packageVersion) {
    // Will run if undefined i.e third argument is not passed
    return new Promise((resolve, reject) => {
        try {
            ...
            packageObj.packageDirectories.forEach((element) => {
                logger.debug(
                    `Trying to copy from ${srcPath}/${element.path}/main/default to ${srcPath}/tempSFDXProject/tempModule/main/default`
                );
                if (fs.existsSync(`${srcPath}/${element.path}/main/default`)) {
                    logger.debug(
                        `${srcPath}/${element.path}/main/default exists  and copying it to ${srcPath}/tempSFDXProject/tempModule/main/default`
                    );
                    fs.copySync(
                        `${srcPath}/${element.path}/main/default`,
                        `${srcPath}/tempSFDXProject/tempModule/main/default`
            ...
            fs.writeJson(
                ...
                (err) => {
                    ...
                    try {
                        // Not adding a try catch here unleashes the zalgo 👹
                        logger.debug(emoji.emojify(":arrow_forward: Artifact creation started......."));
                        // Convert the source
                        shellJS.exec("pwd");
                        ...
                        if (locationToStoreArtifacts) {
                            ...
                            fs.copySync(`${mdapiPackageLocation}`, `${locationToStoreArtifacts}/${mdapiPackageName}`);
                            fs.renameSync(
                        ...
                        zipFolder.zipFolder(
                            `${locationToStoreArtifacts}/${MDAPI_PACKAGE_NAME}-${currentVersion}`,
                            `${locationToStoreArtifacts}/${MDAPI_PACKAGE_NAME}-${currentVersion}.zip`,
                            (error) => {
                                if (error) {
                                    logger.error("Something went wrong!", error);
                                    reject(error);
...
export async function createMDAPIPackageArtifact(projectLocation, artifactsLocation, packageVersion, doGitDiff) {
    return new Promise((resolve, reject) => {
        ...
        if (doGitDiff !== true && doGitDiff !== "true") {
            ...
            fs.renameSync(`${artifactsLocation}/src`, `${artifactsLocation}/${packageName}`);
        }

        createPackageManifest(`${artifactsLocation}/${packageName}`)
            .then((message) => {
                logger.debug(message);
                zipFolder.zipFolder(
                    `${artifactsLocation}/${packageName}`,
                    `${artifactsLocation}/${packageName}.zip`,
                    (error) => {
                        if (error) {
...
export function createPackageVersions(version, oldCommit, newCommit, repoPath) {
