...
export async function refreshBranchFromOrg(accessSecret, srcDir) {
    return new Promise((_, reject) => {
        try {
            ...
            logger.debug(
                "about to execute: " +
                    `sfdx force:mdapi:retrieve -k ${srcDir}/package.xml -r ${BACKUP_DIR} -u ${TARGET_USERNAME} -w 50 --json `
            );
            shellJS.exec(
                `sfdx force:mdapi:retrieve -k ${srcDir}/package.xml -r ${BACKUP_DIR} -u ${TARGET_USERNAME} -w 50 --json `
            );
            ...
            extract(`${path.resolve(BACKUP_DIR, "unpackaged.zip")}`, { dir: `${BACKUP_DIR}` }, (err) => {
                if (err) {
...
export async function retrievePackage(packageName, projectPath, accessSecret) {
    return new Promise((resolve, reject) => {
        ...
        shellJS.exec(sfdxCommand, (status, stdout, stderr) => {
            ...
            if (status === 0) {
                ...
                extract(`${artifactPath}`, { dir: packageDirPath }, (err) => {
                    if (err) {
                        logger.error("retrieve.js: It's an error", err);
                        reject(err);
                    }
...
export async function getElligibleMetadataFiles(folderPath) {
...
export async function handleSpecialMerge(workingDirectory, convertedSrcPath, sfdxModuleName) {
    ...
    const elligibleRetrievedMDFileNames = elligibleRetrievedMDFilesWithFullPath.map((filePath) =>
    ...
    const elligibleMDFilesWithFullPathFromWorkingDir = allFilesInWorkingDirWithFullPath.filter((filePath) => {
        return (
            elligibleRetrievedMDFileNames.includes(path.basename(filePath)) &&
    ...
    fsExtra.copySync(srcFormatPathForRetrievedFiles, srcFormatPathForWDFiles);
    ...
    await convertToMDAPIFormat(srcFormatPathForWDFiles, mdFormatPathForMergedFiles, sfdxModuleName);
    ...
    fsExtra.copySync(mdFormatPathForMergedFiles, workingDirectory, { overwrite: true });
...
export async function convertToSourceInDefaultDestination(source) {
    return await convertToSource(source, "./tmpSFDX");
}

export async function convertToSource(source, destination) {
    // TODO: Exception Handling

    // Convert the mdapi folder into src format
    const output = await shellJS.exec(
        `SFDX_JSON_TO_STDOUT=true sfdx force:mdapi:convert -r "${source}" -d ${destination} --json`
    );
...
export async function mergeToSource(convertedSrcPath, moduleName, filesToIgnore) {
    return new Promise((resolve, reject) => {
...
export async function findFilesToBeIgnoredWhileCopying(source, destination) {
    ...
    return filteredFilesFromSource
        .filter((el) => destFilesBaseNames.includes(path.basename(el)))
        .map((file) => path.basename(file) + "-meta.xml");
}

export async function mergePackageChanges(source, destination, filesToIgnore) {
    return new Promise((resolve, reject) => {
        logger.debug("source: ", source);
        logger.debug("destination: ", destination);
        logger.debug("filesToIgnore: ", filesToIgnore);
...
export async function retrieveCompleteOrg(
    conn,
    rootdir,
    cmpsFolderPath,
    srcFormat,
    deleteBackupDir,
    backUpOnBranch = false
) {
    return new Promise((resolve, reject) => {
        try {
            ...
            createPackageManifest(BACKUP_DIR, true, conn)
                .then((message) => {
                    ...
                    if (srcFormat) {
                        if (!fsExtra.existsSync("sfdx-project.json")) {
                            ...
                            shellJS.cp(
                                path.join(...[rootdir, "temp", "sfdx-project.json"]),
                                path.join(...[rootdir, cmpsFolderPath])
                            );
                        ...
                        shellJS.exec(
                            `sfdx force:source:retrieve -x ${packageXMLPath} -u ${conn.accessToken} --json -w 51`
                        );
                        logger.debug("Project Retrieval Finished");
                    } else {
                        shellJS.exec(
                            `sfdx force:mdapi:retrieve -k ${packageXMLPath} -r ${BACKUP_DIR} -u ${conn.accessToken} -w 51 --json`
                        );

                        //TODO - Henry: Should change out all these joins to use resolve, or else we risk having issues with relative path
                        extract(`${path.join(BACKUP_DIR, "unpackaged.zip")}`, { dir: `${BACKUP_DIR}` }, (err) => {
                            if (err) {
                                logger.error("retrieve.js: It's an error", err);
                                reject(err);
                            }
...
export async function promisifiedParseString(data) {
    return new Promise((resolve, reject) => {
...
export async function convertToRealSFDXFormat(files, destination) {
    let topLevelName = "";
    for (let index = 0; index < files.length; index++) {
        try {
            topLevelName = path.basename(
                path.basename(files[index], ".xml"),
                path.extname(path.basename(files[index], ".xml"))
            );
            ...
            for (const rootElement in result) {
                ...
                for (const firstLevelElmnt in result[rootElement]) {
                    ...
                    if (type(result[rootElement][firstLevelElmnt]) === "Array") {
                        ...
                        for (const element in result[rootElement][firstLevelElmnt]) {
                            ...
                            fsExtra.ensureFileSync(
                                `${destination}/${rootElementName}/${firstLevelElmnt}/${elementName}.xml`
                            );
                            logger.debug(jsonReadyForXMl);
                            // Prepare an individual element xml
                            const xml = builder.buildObject(jsonReadyForXMl);
...
export async function convertToMDAPIFormat(folderPath, destination, sfdxModuleName) {
    ...
    for (let index = 0; index < topLevelDirectories.length; index++) {
        const dirent = topLevelDirectories[index];
        ...
        for (let childIndex = 0; childIndex < firstLevelChildDirs.length; childIndex++) {
            const flc = firstLevelChildDirs[childIndex];
            ...
            const cmpFileName = cmpName
                ? `${cmpName}.${specialMetadataMap[rootName].suffix}-meta.xml`
                : `${specialMetadataMap[rootName].suffix}-meta.xml`;
            logger.debug("cmpFileName: ", cmpFileName);
            logger.debug(
                "cmpFileName path: ",
                path.join(destination, sfdxModuleName, "main", "default", specialMetadataMap[rootName].directoryName)
            );
            ...
            fsExtra.ensureFileSync(
                path.join(
                    ...
                    "main",
                    "default",
                    specialMetadataMap[rootName].directoryName,
                    cmpFileName
                )
            );
