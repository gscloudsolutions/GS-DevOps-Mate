...
const getVersionReport = result => new Promise((resolve, reject) => {
...
const createPackageVersion = command => new Promise((resolve, reject) => {
    try {
        ...
        if (command.targetdevhubusername && command.targetdevhubpassword) {
            authenticate.loginWithCreds(command.targetdevhubusername, command.targetdevhubpassword, 'PRODUCTION')
                .then((connection) => {
...
const handlePackageCodeCoverage = (report, minRequiredCodeCoverage) => new Promise((resolve, reject) => {
    logger.debug('report: ', report);
