...
const compareOrgWithRepo = (orgToCompare, folderLocationToCompare) => new Promise(
  (resolve, reject) => {

  },
);

const compareTwoRepos = (folderLocationOne, folderLocationTwo) => new Promise((resolve, reject) => {
  const options = {
    compareContent: true,
  ...
  dirCompare.compare(sourcePath, targetPath, options)
    .then((res) => {
      ...
      const onlyPathOneFiles = diffSet.filter((element) => {
        if (element.state === 'left'
        && element.type1 === 'file' && element.type2 === 'missing') {
          return element;
        }
