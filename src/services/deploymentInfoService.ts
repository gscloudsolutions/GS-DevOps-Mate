 ...
 */
export function getObject(command) {
...
export async function getDeploymentInfo(moduleName, conn, buildId) {
    return new Promise((resolve, reject) => {
        try {
            ...
            conn.query(
                `SELECT Id, Name, Module_Name__c, Git_Tag__c, Commit_SHA__c,Deployment_Id__c FROM Deployment_Info__c WHERE Module_Name__c = '${moduleName}' LIMIT 1`,
                (err, result) => {
                    if (err) {
                    ...
                    if (buildId && !fs.existsSync(`${pathToStoreDeploymentInfoJSON}/deploymentInfo_${buildId}.json`)) {
                        ...
                        logger.debug(
                            `deploymentInfo.json path with name of the file : ${pathToStoreDeploymentInfoJSON}/deploymentInfo_${buildId}.json`
                        );
                        fs.writeJSONSync(
                            `${pathToStoreDeploymentInfoJSON}/deploymentInfo_${buildId}.json`,
                            result.records[0],
                            { spaces: "\t" }
                        );
                    }
...
export async function updateDeploymentInfo(moduleName, commitSHA, tag, connection, alias, deploymentRes) {
    return new Promise((resolve, reject) => {
        logger.debug(deploymentRes);
        const res = JSON.parse(deploymentRes);
        const result = res.result;

        getDeploymentInfo(moduleName, connection).then((response) => {
            logger.debug(
                `moduleName: ${moduleName}, commitSHA: ${commitSHA}, tag: ${tag}, accessToken: ${connection.accessToken}, alias: ${alias}`
            );
            let targetUserName;
            if (connection) {
            ...
            if (response.totalSize === 1) {
            ...
            } else if (response === NO_SETTINGS_FOUND_ERROR_CODE) {
                ...
                const output = shellJS.exec(
                    `sfdx force:mdapi:deploy -d ${deploymentInfoModulePath} -u ${targetUserName} --json -w -1`
                );
                if (output === "") {
...
export async function updateDeploymentInfoByName(moduleName, commitSHA, targetUserName) {
    return new Promise((resolve, reject) => {
        shellJS.exec(
