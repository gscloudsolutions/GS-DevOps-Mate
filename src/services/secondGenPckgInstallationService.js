...
const findInstalledPackages = params => new Promise((resolve, reject) => {
    try {
        if (params.package) {
            logger.debug('params: ', params);
            resolve(params);
        } else {
            const commandOutput = shellJS.exec(`SFDX_JSON_TO_STDOUT=true sfdx force:package:installed:list -u ${params.orgKey} --json`).stdout;
...
const handlePackageUninstallation = (result, command, resolve, reject) => {
    ...
    if (result === NO_INSTALLED_PACKAGE_FOUND) {
        ...
        commandOutput = shellJS.exec(`SFDX_JSON_TO_STDOUT=true sfdx force:package:uninstall --package ${packageIdentifier} -u ${command.orgKey}  --json -w 21`).stdout;
        const parsedOutput = JSON.parse(commandOutput);
...
const handlePackageInstallation = (result, command, resolve, reject) => {
    ...
    if (result === NO_PACKAGE_FOUND) {
        ...
        logger.debug('result: ', result);
        reject(new Error('Package Identifier you provided is not valid version for installation, due to low code coverage'));
    } else {
        const packageIdentifier = (command.package.toUpperCase() !== LATEST) ? command.package : result.packageId;
        commandOutput = shellJS.exec(`SFDX_JSON_TO_STDOUT=true sfdx force:package:install --package ${packageIdentifier} -u ${command.orgKey}  --json --noprompt -w 21`).stdout;
        const parsedOutput = JSON.parse(commandOutput);
        logger.debug('parsedOutput: ', parsedOutput);
        if (parsedOutput.status === 0) {
...
const uninstallPackage = command => new Promise((resolve, reject) => {
    try {
...
const checkPackageValidityForInstallation = command => new Promise((resolve, reject) => {
    if (!fs.existsSync('./sfdx-project.json')) {
        ...
        reject(new Error('Not a valid sfdx project, make sure to include a valid sfdx-project.json file in the root of the project directory'));
    }
    ...
    for (let index = 0; index < packageAliasesEntries.length; index++) {
        let key = packageAliasesEntries[index][0];
        let value = packageAliasesEntries[index][1];
...
const findLatestValidPackage = command => new Promise((resolve, reject) => {
...
const installPackage = command => new Promise((resolve, reject) => {
    try {
        if (command.targetusername && command.password) {
            authenticate.loginWithCreds(command.targetusername, command.password, command.envtype)
                .then((connection) => {
