...
import { getObject, updateDeploymentInfo } from "../services/deploymentInfoService";
...
} from "../utils/notificationsUtil";
import logger from "../utils/logger";
import resultsTransformer from "../services/deploymentResultsTransformer";
...
export function generateReleaseNotes(jsonObject, locationToStore, type) {
...
const deploy = {
    ...
    success: function (response, uri, validate, minBuildCoverage, minCodeCoveragePerCmp, notificationTitle) {
        ...
        if (!uri) {
            ...
            const codeCoverageResults = calculateOverallCodeCoverage(response, parseInt(minCodeCoveragePerCmp));
    ...
    failed: function (response, uri, validate, minBuildCoverage, minCodeCoveragePerCmp, notificationTitle) {
        ...
        if (!uri) {
            ...
            const codeCoverageResults = calculateOverallCodeCoverage(response, minCodeCoveragePerCmp);
...
const notification = {
    ...
    success: function (uri, codeCoverageResults, validate, notificationTitle) {
        return new Promise((resolve) => {
            return generateSuccessNotificationForSlack(codeCoverageResults, validate, notificationTitle)
                .then((message) => sendNotificationToSlack(uri, message))
                .then((result) => {
                    resolve(result);
                })
                .catch((error) => {
    ...
    failed: function (response, uri, minCodeCoveragePerCmp, codeCoverageResults, validate, notificationTitle) {
        return new Promise((resolve, reject) => {
            return generateFailureNotificationForSlack(
                response,
                minCodeCoveragePerCmp,
                codeCoverageResults,
                validate,
...
export function applyGitTag(tagName, tagMessage) {
    return new Promise((resolve, reject) => {
 ...
 * Author            : Groundswell Cloud Solutions
 */
export const artifactProcessor = {
    ...
    setVersion: function (command, projectPath) {
        ...
        this.version = command.artifactversion
            ...
            : (() => {
                  ...
                  const deploymentInfoObject = getObject(command);
 ...
 * @author           : Groundswell Cloud Solutions
 */
export const deploymentProcessor = {
    /*===============================================
    ...
    updateDeployInfo: function (command, deploymentRes, alias, connection, projectPath) {
        ...
        if (command.successSHA && this.isNotBypass(command) && this.isNotValidation(command)) {
            ...
            return alias
                ? updateDeploymentInfo(command.module, shortSHA, gitTag, null, alias, deploymentRes)
                : updateDeploymentInfo(command.module, shortSHA, gitTag, connection, null, deploymentRes);
