...
import defaultMetadataMappings from "../../config/describeMetadata.json";
...
let metadataMappings = defaultMetadataMappings;

async function getMetadataTypesToIgnore(backupDirPath) {
...
async function getMetadataFilesToIgnore(backupDirPath) {
    return new Promise((resolve, reject) => {
        try {
            const srcDirectoryPath = path.dirname(backupDirPath);
            logger.debug("srcDirectoryPath: ", srcDirectoryPath);
...
export async function fetchMetadataMappings(authTokenOrAlias, latestAPIVersion) {
    return new Promise((resolve, reject) => {
        try {
...
async function getMetadataTypesWithoutSuffix() {
    return new Promise((resolve, reject) => {
        try {
            resolve(
...
async function getMetadataTypesWithFolder() {
    return new Promise((resolve, reject) => {
        try {
            resolve(
                metadataMappings.metadataObjects
...
export async function getMetadataTypesWithMetaFile() {
    return new Promise((resolve, reject) => {
        try {
            resolve(
                metadataMappings.metadataObjects
...
async function createExtensionToCmpNameMapBasedOnCmpNames(files) {
...
async function createExtensionToCmpNameMapBasedOnProjectPath(projectPath) {
    return createExtensionToCmpNameMap((projectPath) => {
        return recursive(projectPath, ["*.xml", ".DS_Store"]);
    }, projectPath);
}

async function createExtensionToCmpNameMap(getFiles, getFilesParam) {
    return new Promise((resolve, reject) => {
...
async function getMetaDataInfoList() {
    return new Promise((resolve, reject) => {
        try {
...
async function listAllMetadata(conn, backupDirPath) {
    try {
        const con = conn;
        const folderTypes = [];
...
async function createTypesFromOrgCmps(conn, backupDirPath) {
...
async function createTypesFromFolderCmps(metadataInfoList, extensionToCmpName) {
    return new Promise((resolve, reject) => {
        try {
            const types = [];
...
function createManifestXML(types) {
    const manifestVersion = process.env.MANIFEST_VERSION || metadataMappings.latestAPIVersion;
    const packageXMLFeed = xmlbuilder.create(
        {
            Package: {
...
function savePackageManifest(xml, projectPath) {
...
export async function saveDestructivePackageManifest(xml, projectPath) {
...
export async function createPackageManifest(projectPath, fullOrg = false, conn = undefined) {
    return new Promise((resolve, reject) => {
...
export async function createDestructiveManifest(files, projectPath) {
