...
import ciCDProviderMessaging from "./cICDProviderSpecificMessaging";
...
export function createProviderSpecificMessage(buildInfo) {
    let buildMessage = "";
...
export async function generateCommonMessage(title, titleStartEmoji, titleEndEmoji) {
    const blocks = [];
...
export async function generateFinalMessage(summary, title, messagePrepFn, startEmoji, endEmoji) {
    const blocks = await generateCommonMessage(title, startEmoji, endEmoji);
    const finalBlocks = await messagePrepFn(blocks, summary);
...
export function calculateOverallCodeCoverage(outputJSON, minCoverage = 75) {
    ...
    castArray(executionResults.runTestResult.codeCoverage).forEach((cmp) => {
        ...
        const codeCoverage = parseInt(
            (
                ((parseInt(cmp.numLocations) - parseInt(cmp.numLocationsNotCovered)) / parseInt(cmp.numLocations)) *
                100
            ).toFixed(2)
        );
    ...
    if (totalNumLocations > 0) {
        return {
            overallBuildCodeCoverage: (
                ((totalNumLocations - totalNumLocationsNotCovered) / totalNumLocations) *
                100
            ).toFixed(2),
...
export function createFailureNotificationForSlack(
    stdout,
...
export async function generateFailureNotificationForSlack(
    stdout,
    minCodeCoveragePerCmp = 75,
    codeCoverageResult,
    isValidation,
    ...
    return ciCDProviderMessaging[ciCDProvider]
        .getBuildInfo()
        .then((buildInfo) =>
...
export function createSuccessNotificationForSlack(codeCoverageResult, isValidation, buildInfo, notificationTitle) {
...
export async function generateSuccessNotificationForSlack(codeCoverageResult, isValidation, notificationTitle) {
    return ciCDProviderMessaging[ciCDProvider]
        .getBuildInfo()
        .then((buildInfo) =>
            createSuccessNotificationForSlack(codeCoverageResult, isValidation, buildInfo, notificationTitle)
        );
}

export function createNoDiffMessage(buildInfo, notificationTitle) {
    const blocks = [];
    ...
    blocks.push({
        ...
        text: {
            type: "mrkdwn",
            text:
                "It can be either there are only non-deployable assets being part of the build like README.md or if that is not the reason, " +
...
export async function generateNoDiffMessage(notificationTitle) {
    return ciCDProviderMessaging[ciCDProvider]
        .getBuildInfo()
        .then((buildInfo) => createNoDiffMessage(buildInfo, notificationTitle));
}

// TODO: This is async, need to handle properly in files calling this method
export async function sendNotificationToSlack(webhook, data) {
