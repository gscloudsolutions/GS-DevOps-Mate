...
import { artifactProcessor, deploymentProcessor } from "../services/deploymentService";
...
const { SF_USERNAME, SF_ENV_TYPE, SF_PASSWORD, GIT_SUCCESS_TAG, SLACK_NOTIFICATION_URI, MIN_OVERALL_CODE_COVERAGE } =
    process.env;
...
program.description("Set of commands to deploy/validate on Salesforce");
...
program
    ...
    .option(
        "-a --targetUserName <tuname>",
        "The username/alias for the target Org that is already authenticated via JWT."
    )
    .option(
        "-c --validate <validate>",
        "Specifies either artifact validation or deployment. If validate, no changes will be deployed to target Org."
    )
    .option(
        "-g --gitTag <tag>",
        "The tag name (annotated not lightweight) to be applied after a successful deployment."
    )
    .option(
        "-l --testlevel <testLevel>",
        "(NoTestRun|RunSpecifiedTests|RunLocalTests|RunAllTestsInOrg) deployment testing level."
    )
    ...
    .action((command) => {
        ...
        const artifact = artifactProcessor
            .checkPath(command)
        ...
        authenticateUser(command, constants)
            .then((con) => {
                ...
                return deploymentProcessor.mdapiDeploy(
                    command,
                    constants,
                    artifact,
                    con.token,
    ...
    .action((command) => {
        ...
        authenticateUser(command, constants)
            .then((con) => {
                ...
                deploymentProcessor.quickDeploy(
                    command,
                    constants,
                    artifact,
                    con.token,
