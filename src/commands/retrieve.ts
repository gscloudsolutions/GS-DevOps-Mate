...
program
    ...
    .action((command) => {
        ...
        loginWithCredentials(command.username.trim(), command.password.trim(), command.type.trim())
            .then((connection) => {
                ...
                return retrievePackage(command.packageName, workingDirectory, connection.accessToken);
            })
            .then((packageLocation) => {
                packagePath = packageLocation;
                return findFilesToBeIgnoredWhileCopying(packagePath, workingDirectory);
            })
            .then((filesToIgnore) => {
                filesToIgnoreList = filesToIgnore;
                return convertToSourceInDefaultDestination(packagePath);
            })
            .then((convertedSrcPath) => handleSpecialMerge(workingDirectory, convertedSrcPath, moduleName))
            .then((convertedSrcPath) => mergeToSource(convertedSrcPath, command.moduleName, filesToIgnoreList))
...
program
    ...
    .action((command) => {
        ...
        let packagePath = "";
        loginWithCredentials(command.username, command.password, command.type)
            .then((connection) => {
                // set the url configuration, required in case of running sfdx commands with access token
                shellJS.exec(`sfdx force:config:set instanceUrl=${connection.instanceURL} --global`);
                if (command.projectLocation) {
            ...
            .then((packageLocation) => {
                packagePath = packageLocation;
                return findFilesToBeIgnoredWhileCopying(packagePath, `${workingDirectory}/${srcDirectory}`);
            })
            .then((filesToIgnore) =>
                mergePackageChanges(packagePath, `${workingDirectory}/${srcDirectory}`, filesToIgnore)
            )
            .catch((error) => {
                console.error("retrieve.js: ", error);
                process.exit(1);
...
program
    ...
    .action(async (command) => {
        try {
            ...
            const AUTH = await loginWithCredentials(command.username, command.password, command.type);
            shellJS.exec(`sfdx force:config:set instanceUrl=${AUTH.instanceURL} --global`);
            await refreshBranchFromOrg(AUTH.accessToken, command.src);
        } catch (error) {
            console.error("retrieve.js: ", error);
            process.exit(1);
        }
...
program
    ...
    .action(async (command) => {
        try {
            ...
            const CONNECTION = await loginWithCredentials(command.username, command.password, command.type);
            shellJS.exec(`sfdx force:config:set instanceUrl=${CONNECTION.instanceURL} --global`);
            await retrieveCompleteOrg(
                CONNECTION,
                command.rootdir,
                mdapiFolderPath,
                false,
...
program
    ...
    .action(async (command) => {
        try {
            ...
            const CONNECTION = await loginWithCredentials(command.username, command.password, command.type);
            shellJS.exec(`sfdx force:config:set instanceUrl=${CONNECTION.instanceURL} --global`);
            await retrieveCompleteOrg(CONNECTION, command.rootdir, mdapiFolderPath, true, deleteBackupDir);
        } catch (error) {
            console.error("retrieve.js: ", error);
            process.exit(1);
        }
