...
import { getDeploymentInfo, updateDeploymentInfoByName } from "../services/deploymentInfoService";

// Various Command Definitions and Actions for Deployment Info Custom Settings
program.description("Set of commands to CRU for Deployment Info Custom Settings");
...
program
    ...
    .description(
        "Updates the commit revision value based on the module name passed for a deployment info custom settings record"
    )
    ...
    .option("-t --envType <type>", "either SANDBOX, PRODUCTION, DEVELOPER or SCRATCH")
    .action((command) => {
        const { module, targetUserName, password, envType, revision } = command;
        if (password) {
            logger.debug("module: ", module);
            if (!command.envType) {
                logger.error("-t --envType is required with username-password based deployment");
            ...
            loginWithCredentials(targetUserName, password, envType)
                .then((connection) => {
