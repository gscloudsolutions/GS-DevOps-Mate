...
import { installPackage, uninstallPackage } from "../services/secondGenPckgInstallationService";

// Various Command Definitions and Actions for Package Installation
program.description("Set of commands to install packages");
...
program
    ...
    .option(
        "-e, --envtype <envtype>",
        "environment type for the target org (required if password is used, SANDBOX|SCRATCH|DEVELOPER|PRODUCTION)"
    )
    .option(
        "-k, --installationkey <key>",
        "installation key for the package (only required if the package needs a key)"
    )
    .action((command) => {
