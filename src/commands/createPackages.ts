...
import { sendNotificationToSlack } from "../utils/notificationsUtil";
import {
    createCombinedArtifact,
    createMDAPIPackageArtifact,
    createMultipleArtifacts,
    createPackage,
} from "../services/packageCreationService";
import {
...
} from "../services/secondGenPackagingService";
...
program
    ...
    .action((command) => {
        createPackage(command, "source-multi", createMultipleArtifacts);
    });
...
program
    ...
    .action((command) => {
        createPackage(command, "source-combined", createCombinedArtifact);
    });
...
program
    ...
    .option("-f --full <full>", "This flag will override everything and go for a full deployment")
    .action((command) => {
        createPackage(command, "mdapi", createMDAPIPackageArtifact);
    });
    ...
    .action((command) => {
        ...
        createPackageVersion(command)
            .then((result) => {
                ...
                return getVersionReport(result);
            })
            .then((report) => {
                logger.debug("Version Reporting Successful");
                logger.debug("report: ", report);
                return handlePackageCodeCoverage(report, command.codecoverage);
            })
            .then((successMessage) => {
                ...
                generateSuccessfulPackageCreationMessage(successMessage)
                    .then((message) => sendNotificationToSlack(SLACK_WEBHOOK_URL, message))
                    .then((message) => {
