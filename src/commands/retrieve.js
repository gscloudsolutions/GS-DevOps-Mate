...
program
    .description('Retrieve unmanaged/managed package or a chageset in source/mdapi format');
    ...
    .action((command) => {
        ...
        authenticate.loginWithCreds(command.username, command.password, command.type)
            ...
            .then((packageLocation) => {
                ...
                return retrieveService.findFilesToBeIgnoredWhileCopying(packagePath, `${workingDirectory}/${srcDirectory}`);
            })
            .then(filesToIgnore => retrieveService.mergePackageChanges(packagePath, `${workingDirectory}/${srcDirectory}`, filesToIgnore))
            .catch((error) => {
                console.error('retrieve.js: ', error);
                process.exit(1);
            });
...
program
    ...
    .option('-p --password <password>', 'Password of org to retrieve from')
    .option('-t --type <type>', 'Type of target org')
    .option('-r --rootdir <rootdir>', 'Path to root directory for the MDAPI formatted folder, mostly the working directory')
    ...
    .option('-b --backupOnBranch <backupOnBranch>', 'Excepts a true or false, should be true if you want the changes to be committed on a branch')
    .action(async (command) => {
        try {
            logger.debug('retrieve.js :: ', 'running :: ');
            ...
            shellJS.exec(`sfdx force:config:set instanceUrl=${CONNECTION.instanceURL} --global`);
            await retrieveService.retrieveCompleteOrg(CONNECTION, command.rootdir, mdapiFolderPath, false, deleteBackupDir, backUpOnBranch);
        } catch (error) {
            console.error('retrieve.js: ', error);
            process.exit(1);
        }
...
program
    ...
    .option('-p --password <password>', 'Password of org to retrieve from')
    .option('-t --type <type>', 'Type of target org')
    .option('-r --rootdir <rootdir>', 'Path to root directory for the SFDX formatted folder, mostly the working directory')
    ...
    .action(async (command) => {
        try {
            ...
            shellJS.exec(`sfdx force:config:set instanceUrl=${CONNECTION.instanceURL} --global`);
            await retrieveService.retrieveCompleteOrg(CONNECTION, command.rootdir, mdapiFolderPath, true, deleteBackupDir);
        } catch (error) {
            console.error('retrieve.js: ', error);
            process.exit(1);
        }
