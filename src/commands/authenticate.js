...
program
    ...
    .action((command) => {
        ...
        authService.loginWithCreds(command.username, command.password, envType)
            .then((response) => {
...
program
    ...
    .action((command) => {
      ...
      || !command.orgAlias || !command.serverKey) {
        ...
        envType = command.envType.toString();
        authService.loginWithJWT(command.clientId, command.serverKey, command.sfUsername, command.orgAlias,
